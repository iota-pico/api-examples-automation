...
async function copyFiles(srcFolder: string, srcFilesGlob: string, destFolder: string, destExt) : Promise<void> {
    try {
        const deletedFiles = await del(path.join(destFolder, "**/*.js"), { force: true });
...
async function copyFile(srcFolder: string, srcFile: string, destFolder: string) : Promise<void> {
    try {
        ...
        await writeFileAsync(path.join(destFolder, srcFile.replace(".ts", ".js")), content);
    } catch (err) {
        console.error(err);
    }
}
